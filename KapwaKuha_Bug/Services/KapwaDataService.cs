...
namespace KapwaKuha.Services
{
    ...
    string securityQuestion, string securityAnswer)
        ...
        LEFT JOIN Post     p ON p.Post_ID     = i.Post_ID";
            ...
            {
                System.Diagnostics.Debug.WriteLine("GetAllItems failed: " + ex.Message);
            }
            return list;
        }
        ...
        public static async Task<List<string>> GetAllCategories()
        {
            ...
            try
            {
                using var conn = new SqlConnection(_conn);
                await conn.OpenAsync();
                using var cmd = new SqlCommand(// Find GetAllCategories or similar — replace the SQL ORDER BY:
"SELECT Category_ID, Category_Name FROM Category ORDER BY CASE WHEN Category_Name = 'Others' THEN 1 ELSE 0 END, Category_Name ASC", conn);
                using var r = await cmd.ExecuteReaderAsync();
                while (await r.ReadAsync())
...
ORDER BY i.Date_Found DESC", conn);
    ...
    GetChatDonors()
     ...
     string orgName = "", string orgAddress = "", string orgContact = "")
        ...
        public static string GenerateClaimReport(ClaimModel claim)
        {
            try
            {
